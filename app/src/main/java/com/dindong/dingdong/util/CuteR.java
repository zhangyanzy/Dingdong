...
public class CuteR {
  ...
  public static Bitmap Product(String txt, Bitmap input, boolean colorful, int color) {
    Log.d(TAG, "Product start input input.getWidth(): " + input.getWidth() + " input.getHeight(): "
        + input.getHeight());
    Bitmap QRImage = null;
    try {
    ...
    scaleQR = scale;
    Bitmap scaledQRImage = Bitmap.createScaledBitmap(QRImage, QRImage.getWidth() * scale,
        QRImage.getHeight() * scale, false);

    int imageSize = 0;
    Bitmap resizedImage = null;
    if (input.getWidth() < input.getHeight()) {
      resizedImage = Bitmap.createScaledBitmap(input, scaledQRImage.getWidth() - scale * 4 * 2,
          (int) ((scaledQRImage.getHeight() - scale * 4 * 2)
              * (1.0 * input.getHeight() / input.getWidth())),
          false);
      imageSize = resizedImage.getWidth();
    } else {
      resizedImage = Bitmap.createScaledBitmap(input,
          (int) ((scaledQRImage.getWidth() - scale * 4 * 2)
              * (1.0 * input.getWidth() / input.getHeight())),
          scaledQRImage.getHeight() - scale * 4 * 2, false);
    ...
    int[][] pattern = new int[scaledQRImage.getWidth() - scale * 4 * 2][scaledQRImage.getWidth()
        - scale * 4 * 2];
    ...
    for (int i = 0; i < patternCenters.length; i++) {
      for (int j = 0; j < patternCenters.length; j++) {
        if (patternCenters[i] == 6 && patternCenters[j] == patternCenters[patternCenters.length - 1]
            || (patternCenters[j] == 6
                && patternCenters[i] == patternCenters[patternCenters.length - 1])
    ...
    Log.d(TAG, "Product end input scaledQRImage.getWidth(): " + scaledQRImage.getWidth()
        + " scaledQRImage.getHeight(): " + scaledQRImage.getHeight());
  ...
  public static Bitmap ProductEmbed(String txt, Bitmap input, boolean colorful, int color, int x,
      int y, Bitmap originBitmap) {
    ...
    qrBitmap = resizeQuiteZone(qrBitmap, newScale); // it does not match QR spec to cut the qr quiet
                                                    // zone
  ...
  private static Bitmap resizeQuiteZone(Bitmap qrBitmap, double scale) {
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        if (i < boundary || i > size - (boundary + 1) || j < boundary
            || j > size - (boundary + 1)) {
  ...
  public static Bitmap ProductNormal(String txt, boolean colorful, int color) {
    ...
    return Bitmap.createScaledBitmap(QRImage, QRImage.getWidth() * SCALE_NORMAL_QR,
  ...
  public static Bitmap ProductLogo(Bitmap logo, String txt, boolean colorful, int color) {
    ...
    if (finalSize > fullSize) {
      ...
      qrImage = Bitmap.createScaledBitmap(qrImage, qrImage.getWidth() * finalSize / fullSize,
    ...
    Bitmap white = Bitmap.createBitmap(background - boundary * 2, background - boundary * 2,
    ...
    canvasQR.drawBitmap(gray, scale * 4 + (fullSize - background) / 2,
        scale * 4 + (fullSize - background) / 2, null);
    canvasQR.drawBitmap(scaleLogo, scale * 4 + (fullSize - logoSize) / 2,
        scale * 4 + (fullSize - logoSize) / 2, null);
    return qrImage;
  }
  ...
  public static Bitmap[] ProductGIF(String txt, Bitmap[] input, boolean colorful, int color) {
    ...
    for (Bitmap inputBitmap : input) {
      if (size > MAX_INPUT_GIF_SIZE) {
        inputBitmap = Bitmap.createScaledBitmap(inputBitmap, MAX_INPUT_GIF_SIZE, MAX_INPUT_GIF_SIZE,
            false);
      }
    ...
    //// }
    //// }
    // Bitmap white = Bitmap.createBitmap(maxSize, maxSize,
    // Bitmap.Config.ARGB_8888);
    ...
    // }
    // Bitmap resizeImage = Bitmap.createScaledBitmap(inputBitmap,
    // (int)(resizedImageSize*scale), (int)(resizedImageSize*scale), false);
    // Bitmap finalImage = Bitmap.createBitmap(whiteQR.getWidth(),
    // whiteQR.getHeight(), Bitmap.Config.ARGB_8888);
    // Canvas canvasQR = new Canvas(finalImage);
    // canvasQR.drawColor(Color.WHITE);
    // canvasQR.drawBitmap(resizeImage, scaleQR * 4 + (resizedImageSize -
    // (int)(resizedImageSize*scale))/2, scaleQR * 4 + (resizedImageSize -
    // (int)(resizedImageSize*scale))/2, null);
  ...
  public static Bitmap replaceColor(Bitmap qrBitmap, int color) {
    ...
    qrBitmap.getPixels(allpixels, 0, qrBitmap.getWidth(), 0, 0, qrBitmap.getWidth(),
        qrBitmap.getHeight());
    ...
    qrBitmap.setPixels(allpixels, 0, qrBitmap.getWidth(), 0, 0, qrBitmap.getWidth(),
        qrBitmap.getHeight());
    return qrBitmap;
  }
  ...
  public static Bitmap encodeAsBitmap(String txt) throws WriterException {
    ...
    QRCode qrCode;
    try {
      qrCode = Encoder.encode(contentsToEncode, ErrorCorrectionLevel.M, hints);
